...
//  // C0-C4 (state index 8-11)
    ...
    if ((tickCount / 1000) <= SecWinUptime)
    {
        ...
        if (!NoWaitMessage)
        {
            ...
            else MessageBox(NULL, bufsec, "TPFanControl is sleeping", MB_ICONEXCLAMATION);
        }
    }
    ...
    if (this->MinimizeToSysTray)
    {
        ...
        if (!this->ShowTempIcon) this->pTaskbarIcon = new TASKBARICON(this->hwndDialog, 10, "TPFanControl");
        else this->pTaskbarIcon = NULL;
    }
    ...
    if (this->StartMinimized) ::ShowWindow(this->hwndDialog, SW_MINIMIZE);
}
...
FANCONTROL::~FANCONTROL()
{
    ...
    if (this->hwndDialog) ::DestroyWindow(this->hwndDialog);
...
int FANCONTROL::CurrentModeFromDialog()
{
    ...
    if (modetpauto) this->CurrentMode = 1;
    else
    {
        if (modefcauto) this->CurrentMode = 2;
        else
        {
            if (modemanual) this->CurrentMode = 3;
            else this->CurrentMode = -1;
        }
    }
...
//if running as service do not end
    ...
    case WM__TASKBAR:
        ...
        case WM_LBUTTONUP:
            //BOOL isshift = ::GetAsyncKeyState(VK_SHIFT) & 0x8000, isctrl= ::GetAsyncKeyState(VK_CONTROL) & 0x8000;
            //int action = -1;
        ...
        case WM_RBUTTONDOWN:
        {
            ...
            {
                if (mode == 2) m.CheckMenuItem(5002);
            }

            if (mode == 3) m.CheckMenuItem(5005);
            ...
            if (this->SmartLevels2[0].temp2 != 0)
            {
                ...
                if (mode == 2 && this->IndSmartLevel == 0) m.CheckMenuItem(5003);

                if (mode == 2 && this->IndSmartLevel != 0) m.CheckMenuItem(5004);
            }

            if (Runs_as_service) m.DeleteMenuItem(5020);

            if (!IsWindowVisible(this->hwndDialog)) m.DeleteMenuItem(5030);

            if (IsWindowVisible(this->hwndDialog)) m.DeleteMenuItem(5010);

            if (this->ShowTempIcon == 0) m.DeleteMenuItem(5070);

            if (this->ShowTempIcon == 1) m.DeleteMenuItem(5080);
...
void FANCONTROL::ProcessTextIcons(void)
{
    ...
    if (pTextIconMutex->Lock(100))
    {
        ...
        for (int i = 0; i < MAX_TEXT_ICONS; ++i)
        {
            if (ppTbTextIcon[i])
            {
                if (Fahrenheit) _itoa_s((this->icontemp * 9 / 5) + 32, str_value, sizeof(str_value), 10);
                else _itoa_s(this->icontemp, str_value, sizeof(str_value), 10);
