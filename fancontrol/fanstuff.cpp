...
//    Usage of binaries or source shall be entirely and 
...
FANCONTROL::HandleData(void)
{
    ...
    for (i = 0; i < 12; ++i)
    {
        ...
        if (this->State.Sensors[i] != 0x80 && this-State.Sensors[i] != 0x00 && strstr(list, what) == 0)
        {
            ...
            if (senstemp < 128)
            {
                ...
                if (maxtemp <= senstemp)
                {
                    imaxtemp = i;  // this->State.SensorName[this->iMaxTemp]
                }
            }
        }
    }
    ...
    else
    {
        ...
        else
        {
            ...
            sprintf_s(title2 + strlen(title2), sizeof(title2) - strlen(title2), " Fan %d (%s)",
                      fanctrl & 0x3F, this->CurrentModeFromDialog() ==2 ? "Smart" : "Fixed");
        }
    }
    ...
    if (this->CurrentMode == 3 && this->MaxTemp > 75)
    {
        this->CurrentMode = 2;
    }
...
int FANCONTROL::SmartControl(void)
{
    ...
    {
        // TODO: Collecting data for Graph
        //
        // if (newfanctrl == 0x80)
        // {
...
int FANCONTROL::SetFan(const char *source, int fanctrl, BOOL final)
{
    ...
    {       
        ...
        for (int i = 0; i < 5; ++i)
        {
            // *** BUGBUG: handle Fan2 in a different loop.
...
int FANCONTROL::ReadEcStatus(FCSTATE *pfcstate)
{
    ...
    FCSTATE sample = {0};  // Modification: "{0, }" wg. fanspeed transient zero

    // setzero(pfcstate, sizeof(*pfcstate)); // Modification: wg. fanspeed transient zero
        
    // reading from the EC seems to yield erratic results at times (probably
    // due to collision with other drivers reading from the port). 
    // So try up to three times to read two samples which look oka and have matching value.
...
try_again:
        ...
        if (ivers >= 3) 
        {
         ...
         for (iTime = 0; iTime < iTimeoutBuf; iTime += iTick)
         {
             ...
             if (data & 0x50)                             // some unrequested output is waiting 
             {
