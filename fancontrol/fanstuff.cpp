...
int FANCONTROL::HandleData(void)
{
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        if (this->State.Sensors[i] != 0x80 && this - State.Sensors[i] != 0x00 && strstr(list, what) == 0)
        {
            ...
            if (ShowBiasedTemps) senstemp = isens;
            else senstemp = isens - ioffs;
    ...
    if (this->SlimDialog == 1)
    {
        ...
        if (fanctrl & 0x80)
        {
            if (!(SlimDialog && StayOnTop)) strcat_s(obuf2, sizeof(obuf2), "(= BIOS)");
            strcat_s(title2, sizeof(title2), " Default Fan");
        }
        else
        {
            if (!(SlimDialog && StayOnTop)) sprintf_s(obuf2 + strlen(obuf2), sizeof(obuf2) - strlen(obuf2), "%d Non Bios", fanctrl & 0x3F);
    ...
    if (Fahrenheit) sprintf_s(obuf2, sizeof(obuf2), "%d°F", this->MaxTemp * 9 / 5 + 32);
    else sprintf_s(obuf2, sizeof(obuf2), "%d°C", this->MaxTemp);
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        if (temp < 128 && temp != 0)
        {
            if (Fahrenheit) sprintf_s(obuf2, sizeof(obuf2), "%d°F", temp * 9 / 5 + 32);
            else sprintf_s(obuf2, sizeof(obuf2), "%d°C", temp);
    ...
    if (Fahrenheit)
    {
        for (i = 0; i < 12; i++)
        {
            ...
            else strcat_s(templist, sizeof(templist), "0;");
        }
    }
    else
    {
        for (i = 0; i < 12; i++)
        {
            if (this->State.Sensors[i] != 128) sprintf_s(templist + strlen(templist), sizeof(templist) - strlen(templist), "%d; ", this->State.Sensors[i]);
            else strcat_s(templist, sizeof(templist), "0; ");
        }
    }
    templist[strlen(templist) - 1] = '\0';

    if (Fahrenheit) sprintf_s(CurrentStatus, sizeof(CurrentStatus), "Fan: 0x%02x / Switch: %d°F (%s)", State.FanCtrl, MaxTemp * 9 / 5 + 32, templist);
    else sprintf_s(CurrentStatus, sizeof(CurrentStatus), "Fan: 0x%02x / Switch: %d°C (%s)", State.FanCtrl, MaxTemp, templist);
    ...
    case 1: // Bios Auto
        ...
        {
            this->PrintModeChange(obuf);
        }

        if (this->State.FanCtrl != 0x080) ok = this->SetFan("BIOS", 0x80);
        break;
    ...
    case 3: // fixed manual
        ...
        {
            if (this->State.FanCtrl != atoi(manlevel)) ok = this->SetFan("Manual", atoi(manlevel));
            else ok = true;
        }
        break;
    }
...
void FANCONTROL::PrintModeChange(char *obuf)
{
...
//-------------------------------------------------------------------------
int FANCONTROL::SmartControl(void)
{
    int ok = 0, i, newfanctrl = -1, fanctrl = this->State.FanCtrl;
    char obuf[256] = "";
...
bool FANCONTROL::SetFan(const char* source, int fanctrl, BOOL final)
{
    ...
    if (this->ActiveMode && !this->FinalSeen)
    {
        ...
        if (!ok) this->Trace("could not set level of fan 1!");
        ...
        if (!ok) this->Trace("could not set level of fan 2!");
        ...
        if (this->State.FanCtrl == fanctrl)
        {
            ...
            ok = true;
            if (final) this->FinalSeen = true;// prevent further changes when setting final mode
        }
        else
        {
...
int FANCONTROL::SetHdw(const char* source, int hdwctrl, int HdwOffset, int AnyWayBit)
{
    int ok = 0;
    char obuf[256] = "", obuf2[256], datebuf[128];
    char newhdwctrl;
...
//-------------------------------------------------------------------------
int FANCONTROL::ReadEcStatus(FCSTATE* pfcstate)
{
    DWORD ok = 0, rc = 0;
...
//  read fan and temperatures from embedded controller
//-------------------------------------------------------------------------
int FANCONTROL::ReadEcRaw(FCSTATE* pfcstate)
{
    int i, idxtemp;
    pfcstate->FanCtrl = -1;
    ...
    neuerversuch:
        ...
        for (iTime = 0; iTime < iTimeoutBuf; iTime++)
        {           // wait for full buffers to clear
            data = (char)ReadPort(0x1604) & 0xff; // or timeout iTimeoutBuf = 1000
            if (data == 0x50) break;
        }

        if (data != 0x50) goto neuerversuch;
