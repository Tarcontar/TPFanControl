...
int FANCONTROL::HandleData(void)
{
	...
	if (this->SlimDialog == 1)
	{
		...
		{
			if (!(SlimDialog && StayOnTop)) sprintf_s(obuf2+strlen(obuf2),sizeof(obuf2)-strlen(obuf2), " Non Bios", fanctrl & 0x3F);
...
//-------------------------------------------------------------------------
int FANCONTROL::SetFan(const char *source, int fanctrl, BOOL final)
{
	int ok= 0;
...
//	pfcstate->FanSpeedHi= 0;
	...
	if (!ok) this->Trace("failed to read FanSpeedHighByte from EC");
