...
//  read config file
//-------------------------------------------------------------------------
int FANCONTROL::ReadConfig(const char* configfile)
{
    char buf[1024];
    ...
    if (!errf)
    {
        while (!feof(f))
        {
            ...
            else if (_strnicmp(buf, "MenuLabelSM1=", 13) == 0)
            {
                ...
                while (*p != '/')
                {   // copy until '/' excluding tab, carr ret, new line
//                      if (*p!='*') continue; else break;
    ...
    if (Fahrenheit)
    {
        ...
        for (i = 0; this->SmartLevels[i].temp != -1; i++)
        {
            ...
            else strcat_s(buf, sizeof(buf), "0x80");
        }
    }
    else
    {
    ...
    if (Fahrenheit && (this->ManModeExit == 80)) this->ManModeExit = (this->ManModeExit * 9 / 5) + 32;

    if (Fahrenheit) this->ManModeExit2 = (this->ManModeExit - 32) * 5 / 9;
    else this->ManModeExit2 = this->ManModeExit;
...
//  localized date&time
//-------------------------------------------------------------------------
void FANCONTROL::CurrentDateTimeLocalized(char* result, size_t sizeof_result)
{
    SYSTEMTIME s;
    ::GetLocalTime(&s);
...
//  localized time
//-------------------------------------------------------------------------
void FANCONTROL::CurrentTimeLocalized(char* result, size_t sizeof_result)
{
    SYSTEMTIME s;
    ::GetLocalTime(&s);
...
//  
//-------------------------------------------------------------------------
BOOL FANCONTROL::IsMinimized(void)
{
...
{
    char trace[10000] = "", datebuf[128] = "", line[256] = "", linecsv[256] = "";
...
void FANCONTROL::Tracecsv(const char* text)
{
    char trace[10000] = "", datebuf[128] = "", line[256] = "";

    this->CurrentTimeLocalized(datebuf, sizeof(datebuf));

    if (strlen(text)) sprintf_s(line, sizeof(line), "%s; %s\r\n", datebuf, text);    // probably acpi reading conflict
    else strcpy_s(line, sizeof(line), "\r\n");
...
void FANCONTROL::Tracecsvod(const char* text)
{
    char trace[10000] = "", datebuf[128] = "", line[256] = "";

    this->CurrentDateTimeLocalized(datebuf, sizeof(datebuf));

    if (strlen(text)) sprintf_s(line, sizeof(line), "%s\r\n", text); // probably acpi reading conflict
    else strcpy_s(line, sizeof(line), "\r\n");
...
//  create a thread
//-------------------------------------------------------------------------
HANDLE FANCONTROL::CreateThread(int(_stdcall* fnct)(ULONG), ULONG p)
{
